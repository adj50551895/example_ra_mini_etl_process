...
print("Processing hdp.spark_event_activity_b_logins ...")
# 1 content_detail
...
join ids_coredata.dim_date dd on date(dd.date_date) = date(c.event_date)
where 1=1
...
df_query_spark_event_activity_logins = load_data_from_databricks.get_data_from_sql_query(query_spark_event_activity_logins)
print("No. of rows (df_query_spark_event_activity_logins): ", df_query_spark_event_activity_logins.shape[0])
...
print("Processing hdp.ccmusg_fact_user_activity_cc_dc ...")
# 2 content_detail
...
from ocf_analytics.fact_user_activity b
join ccanalytics.ra_edex_member_guid_map mp on mp.userguid = b.member_guid
join ids_coredata.dim_date dd on date(dd.date_date) = date(b.activity_date)
...
df_ccmusg_fact_user_activity = load_data_from_databricks.get_data_from_sql_query(query_ccmusg_fact_user_activity)
print("No. of rows (df_ccmusg_fact_user_activity): ", df_ccmusg_fact_user_activity.shape[0])

...
import core.utils.load_data_from_databricks as load_data_from_databricks
...
print("Processing hdp.mcietl_web_visits_detailed_edex_clicks ...")
# 1 #dash report
...
from acometl_shared.web_visits_detailed t
join ids_coredata.dim_date dd on date(dd.calendar_date) = t.click_date
left join ocf.sc_visid_guid_unique sc on cast(sc.visid_high as bigint) = t.visid_high and cast(sc.visid_low as bigint) = t.visid_low
...
df_edex = load_data_from_databricks.get_data_from_sql_query(query_url_edex_adobe_com)
print("No. of rows (df_edex): ", df_edex.shape[0])
...
print("Processing hdp.mcietl_web_visits_detailed_express_clicks ...")
# 2
...
from acometl_shared.web_visits_detailed wb
join ids_coredata.dim_date dd on date(dd.calendar_date) = wb.click_date
...
df_express = load_data_from_databricks.get_data_from_sql_query(query_url_express_adobe_com)
print("No. of rows (df_express): ", df_express.shape[0])
...
#comparison_df = df_edex.merge(df_edex_1, indicator=True, on=["visid_high", "visid_low", "visit_num", "click_date"], how='outer')
